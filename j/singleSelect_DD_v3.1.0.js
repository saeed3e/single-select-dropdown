/*global ieObj*/
/*jslint eqeq:true*/
/*******Start of singleDD (version : v.3.1.0)*/
			...
			function init(node){
				...
				_t.replaceData = function(obj){
					if(obj.prefillData){
						...
						(obj.isReset!=false)?_t.setVal_inHiddenField('',''):'';
					}
					_t.dropCont.html(_t.appendData(obj.data));
				};
						...
						if(_t.currentActive){
							...
							if(opts.changeDirection){
								...
								if(wrap == null){
									...
									eleTop = elm.find('.dWrap').offset().top - $(window).scrollTop(); //Element distance from top
								}
								else{
									mainEle = $(wrap).height();
									eleTop = $(wrap).scrollTop() + elm.find('.dWrap').offset().top //Element distance from parent
								}
								var width = opts.width && opts.width!='auto'?opts.width:elm.width()+'px';
								if(mainEle>=opts.maxHeight){
									if(mainEle>=(eleTop+opts.maxHeight)){
										_t.dropCont.css({top:elm.find('.dWrap').outerHeight(), left:0, display:'block'});
									}
									else{
